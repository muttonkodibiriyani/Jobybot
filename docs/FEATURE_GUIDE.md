...
Run `DASHBOARD.bat` (Win) or `mac/Dashboard.command` and your browser
opens `data/dashboard.html`. **`RUN_BOT_NOW.bat` also opens it
automatically before the cycle starts** so you always have two windows
open: the terminal log and the visual dashboard.

You'll see:

- **🟢 Live banner at the top** — plain-English status of what the bot is
  doing *right now*: "🔎 Searching all 7 sources right now", "📨 Just
  sent: careers@careem.com (Careem)", "💤 Idle — bot not running". The
  green dot pulses while the bot is active.
- **KPI tiles**: Emails today / cap, Jobs today, Total emails sent, Bounces.
- **Jobs by source**: counts of jobs found per LinkedIn / Indeed / etc.
- **Recent activity (last 100 events)**: a live audit trail of every
  search call, AI score, email send, bounce, market-skip, cap-hit.
- **Top matched jobs (click to apply)**: top 50 by Gemini score with
  direct Open buttons.
- **Recent bounces / invalid addresses**: which emails bounced, why,
  and when — never retried.

Refreshes every **15 seconds**. The dashboard runs on your laptop only —
nobody else can see it without your file path. No login, no internet.

> **It's just an HTML file.** `data/dashboard.html` is regenerated by the
> bot every few seconds while a cycle is running, and every 15 seconds
> the browser tab reloads it. No web server, no port, no auth — perfect
